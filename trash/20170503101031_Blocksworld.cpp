...
vector<SmartVector> Blocksworld::Get_Action_List(SmartVector state) const
{
	...
	// Case #2: UNSTACK (j,s) where j<0, s={0,1,...,n-1}
	// 		- Step1: Get the block from top of the slot if there is more than a single block.
	// 		- Step2: Put back to the first free slot.
	// Case #3: STACK (i,s) where i>0, s={0,1,...,n-1}
	//		- Step1: Get the block from top of the slot in which it is located.
	//		- Step2: Release it to the top of the another give slot (s) that is not empty.
	// 		- Note: Since our state doesn't hold slot number for blocks, it doesn't make a sence
	//		to remove a singe block from one slot to another empty slot. Yet we include it,
...
SmartVector Blocksworld::Get_Next_State(const SmartVector& state, const SmartVector& action)
{
	...
	// Case #2: UNSTACK (j,s) where j<0, s={0,1,...,n-1}
	// 		- Step1: Get the block from top of the slot if there is more than a single block.
	// 		- Step2: Put back to the another non-empty block or first free slot.
	// Case #3: STACK (i,s) where i>0, s={0,1,...,n-1}
	//		- Step1: Get the block from top of the slot in which it is located.
	//		- Step2: Release it to the top of the another give slot (s) that is not empty.
	// 		- Note: Since our state doesn't hold slot number for blocks, it doesn't make a sense
	//		to remove a singe block from one slot to another empty slot.
...
void Blocksworld::Display_Action(const SmartVector& action) const
{
	...
	if(action.elements[0] < 0)
		cout << "UnStack Block #"<<  block_id << " to Slot #" << slot_id+1 << endl;

	if(action.elements[0] > 0)
		cout << "Stack Block #"<<  block_id << " to Slot #" << slot_id+1 << endl;
}
...
void Blocksworld::Display_Policy(const Representation& rep)  const
{
	...
	for (unsigned i = 0; i < States.size(); ++i)
	{
		...
		for (unsigned j = 0; j < Actions.size(); ++j) {
			...
			if(action.elements[0] < 0)
				cout << "UnStack Block #"<<  block_id << " to Slot #" << slot_id+1 << "\t";

			if(action.elements[0] > 0)
				cout << "Stack   Block #"<<  block_id << " to Slot #" << slot_id+1 << "\t";
